    ...
    <script>
        ...
        class Renderer {
            ...
            render(env) {
                ...
                ctx.lineTo(leftBoundary, height * 0.7);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(rightBoundary, height * 0.5);
                ctx.lineTo(rightBoundary, height * 0.7);
                ctx.stroke();
                ctx.setLineDash([]);
                ...
                ctx.fillStyle = '#333';
                ctx.beginPath();
                ctx.arc(cartX - cartWidth / 3, cartY, 8, 0, Math.PI * 2);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(cartX + cartWidth / 3, cartY, 8, 0, Math.PI * 2);
                ctx.fill();
                ...
                ctx.globalAlpha = 0.5;
                ctx.beginPath();
                ctx.arc(cartX, cartY - cartHeight, 30, -Math.PI / 2, -Math.PI / 2 + env.theta, env.theta > 0);
                ctx.stroke();
                ctx.globalAlpha = 1.0;
                ...
                if (Math.abs(env.xDot) > 0.1) {
                    ...
                    ctx.lineTo(cartX + env.xDot * velocityScale - Math.cos(angle - Math.PI / 6) * 8,
                              cartY - cartHeight / 2 - Math.sin(angle - Math.PI / 6) * 8);
                    ctx.lineTo(cartX + env.xDot * velocityScale - Math.cos(angle + Math.PI / 6) * 8,
                              cartY - cartHeight / 2 - Math.sin(angle + Math.PI / 6) * 8);
                    ctx.closePath();
                ...
                // Warning indicators
                if (Math.abs(env.x) > env.xThreshold * 0.7 || Math.abs(env.theta) > env.thetaThreshold * 0.7) {
        ...
        class RLSandbox {
            constructor() {
                ...
                this.exploration = 0.2; // Start with more exploration
                this.explorationDecay = 0.998; // Much slower decay
                this.minExploration = 0.05; // Higher minimum
            ...
            async runEpisode(train = false) {
                ...
                while (!done && (this.isTraining || this.isTesting)) {
                    ...
                    if (!result.done) {
                        ...
                        shapedReward -= 0.05 * Math.abs(this.env.theta);

                        // Tiny penalty for being off-center
                        shapedReward -= 0.001 * Math.abs(this.env.x);
                    } else {
                ...
                if (this.survivalHistory.length > 100) {
            ...
            resetPolicy() {
                ...
                this.episode = 0;
                this.survivalHistory = [];
                this.exploration = 0.2; // Reset exploration
                this.chart.clear();
                this.initNetwork();
                this.env.reset();
                this.updateMetrics();
            ...
            updateActionBars() {
                if (this.network.lastOutput) {
                    ...
                    // Update left action bar
                    const leftPct = (probs[0] * 100).toFixed(1);
                    document.getElementById('action-left').style.height = leftPct + '%';
                    document.getElementById('action-left-val').textContent = leftPct + '%';

                    // Update right action bar
                    const rightPct = (probs[1] * 100).toFixed(1);
                    document.getElementById('action-right').style.height = rightPct + '%';
            ...
            updateStateDisplay() {
                ...
                const maxX = this.env.xThreshold;
                const maxXDot = 4.0;
                const maxTheta = this.env.thetaThreshold;
                const maxThetaDot = 4.0;
